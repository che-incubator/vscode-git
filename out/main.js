...
async function createModel(context, outputChannel, telemetryReporter, disposables) {
    ...
    let ipc;
    try {
        ipc = await ipcServer_1.createIPCServer();
    ...
    if (ipc) {
        const askpass = new askpass_1.Askpass(ipc);
        disposables.push(askpass);
        env = { ...env, ...askpass.getEnv() };
    }
    else {
        env = { ...env, ...askpass_1.Askpass.getDisabledEnv() };
    }
    ...
    disposables.push(new commands_1.CommandCenter(git, model, outputChannel, telemetryReporter), new contentProvider_1.GitContentProvider(model), new fileSystemProvider_1.GitFileSystemProvider(model), new decorationProvider_1.GitDecorations(model), new protocolHandler_1.GitProtocolHandler(), new timelineProvider_1.GitTimelineProvider(model));
